        ...
        if (b = document.getElementById("inhalt")) {
            ...
            if (e = b.querySelector("#missions")) {
                ...
                if (f = b.querySelector("#arrivalTime")) {
                    f.setAttribute("original", f.textContent), f.textContent = AGO.Time.convertLocal(f.textContent, "[H]:[i]:[s]"), DOM.setStyleDisplay(f.parentNode.parentNode), DOM.appendSPAN(d, "ago_fleet_name", DOM.getTextChild(f.parentNode.parentNode)), g = DOM.appendSPAN(d, "ago_fleet_arrival"), g.appendChild(f);
                }
                d = DOM.appendDIV(c, {
